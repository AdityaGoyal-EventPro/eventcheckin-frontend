  ...
  const onScanSuccess = (decodedText, decodedResult) => {
    ...
    try {
      ...
      if (qrData.guest_id && qrData.event_id) {
        ...
        console.log('Calling onScan with:', qrData);
        setSuccess(`Scanned: ${qrData.guest_name || 'Guest'}`);
        
        // Stop scanner first
        stopScanner().then(() => {
          console.log('Scanner stopped, now calling onScan callback');
          // Call the callback
          onScan(qrData);
        }).catch(err => {
          console.error('Error stopping scanner, calling onScan anyway');
          onScan(qrData);
        });
      } else {
